...
def engage_inbox(request):
    ...
    from apps.messaging.engage.lead_escalation import escalate_flagged_threads
    ...
    interactions = list(interactions.all()[:50])
    if highlight_id:
        highlighted = (
            request.user.interactions.select_related("social_account", "post")
            .filter(pk=highlight_id)
            .first()
        )
        if highlighted and all(str(i.pk) != str(highlighted.pk) for i in interactions):
            interactions.insert(0, highlighted)
        elif highlighted:
            interactions = [highlighted] + [i for i in interactions if i.pk != highlighted.pk]
...
def trigger_engage(request):
    ...
    from apps.create.agents.engage_agent import run_engage_cycle
...
def unified_needs_reply(request):
    ...
    if source_filter not in ("all", "whatsapp", "social", "dms"):
        source_filter = "all"

    items = []

    wa_convs = WhatsAppConversation.objects.none()
    if source_filter in ("all", "whatsapp"):
        wa_convs = (
        ...
        for conv in wa_convs:
            ...
            items.append({
                ...
                "url": reverse("whatsapp:conversation", kwargs={"pk": conv.pk}),
            })

    engage_qs = Interaction.objects.none()
    if source_filter in ("all", "social", "dms"):
        engage_qs = (
            request.user.interactions.filter(status__in=["new", "flagged"])
            .select_related("social_account")
            .order_by("-created_at")
        )
        if source_filter == "dms":
            engage_qs = engage_qs.filter(
                interaction_type=Interaction.InteractionType.DM,
            ).exclude(platform="whatsapp")
        elif source_filter == "social":
            engage_qs = engage_qs.exclude(
                interaction_type=Interaction.InteractionType.DM,
            )
        engage_qs = engage_qs[:25]
        for interaction in engage_qs:
            if interaction.platform == "whatsapp":
                item_url = reverse("whatsapp:inbox")
            elif interaction.interaction_type == Interaction.InteractionType.DM:
                item_url = (
                    reverse("engage:dm_inbox")
                    + f"?platform={interaction.platform}"
                    + (
                        f"&sender={interaction.author_username}"
                ...
                item_url = (
                    reverse("engage:inbox")
                    + f"?needs_reply=1&highlight={interaction.pk}"
                )
            items.append({
                "source": "engage",
                "kind": (
                    "dm"
                    if interaction.interaction_type == Interaction.InteractionType.DM
                    else "social"
                ),
    ...
    stats = {
        "total": len(items),
        "wa_count": sum(1 for i in items if i["source"] == "whatsapp"),
        "engage_count": sum(1 for i in items if i["source"] == "engage"),
    }
...
def messenger_threads(request):
    ...
    from apps.messaging.engage.dm_inbox import get_dm_threads
...
def dm_inbox_view(request):
    """DM inbox for Facebook/Instagram (WhatsApp chats live under WhatsApp)."""
    ...
    dm_interactions = request.user.interactions.filter(
        interaction_type=Interaction.InteractionType.DM,
    ).exclude(platform="whatsapp").select_related("social_account")
    ...
    # WhatsApp is handled in the WhatsApp workspace — hide from DM platform chips.
    platforms = sorted({t["platform"] for t in threads if t["platform"] != "whatsapp"})
    threads = [t for t in threads if t["platform"] != "whatsapp"]
