...
from apps.create.agents.llm import coerce_llm_dict, generate, get_model_for_task, parse_llm_json
...
def handle_incoming_message(self, message_id: str):
    ...
    """
    from apps.create.agents.engage_routing import RoutingAction, SafetyContext, route_reply, safety_check
    ...
    try:
        from apps.messaging.whatsapp.commerce import handle_commerce_message
    ...
    try:
        from apps.messaging.whatsapp.memory import remember_customer_interaction
    ...
    if routing == RoutingAction.AUTO_SEND:
        ...
        try:
            from apps.messaging.whatsapp.draft_actions import notify_owner_draft_created
...
def _build_system_prompt(profile, conversation):
    ...
    try:
        from apps.messaging.whatsapp.memory import memory_context_block, recall_customer_memory
...
def generate_status_content(self, user_id: int, prompt: str, category: str):
    ...
    from apps.messaging.whatsapp.models import StatusContent
...
def execute_broadcast(self, broadcast_id: str):
    ...
    """
    from apps.messaging.whatsapp.models import WhatsAppBroadcast
    from apps.messaging.whatsapp.services import send_template_message, variables_to_components
    ...
    from apps.core.billing.whatsapp_marketing import check_whatsapp_marketing_limit, is_marketing_template
...
def process_sequence_steps():
    """V1 no-op — broadcast sequences UI/beat removed."""
    return {"processed": 0, "skipped": "whatsapp_sequences_v1_disabled"}
...
def generate_weekly_digest():
    ...
    """
    from apps.core.accounts.models import UserProfile
    from apps.messaging.whatsapp.models import WeeklyDigest, WhatsAppAnalytics
    from apps.core.platforms.models import SocialAccount
    from datetime import timedelta
...
def cross_post_to_channel(self, channel_id: str, post_id: str):
    ...
    """
    from apps.create.content.models import Post
    from apps.messaging.whatsapp.models import ChannelPost, WhatsAppChannel
