...
def shopify_order_webhook(request):
    ...
    """
    from apps.insight.analytics.models import Conversion, ShopifyStore
...
def mpesa_commerce_callback(request):
    ...
    if commerce_payment:
        ...
        try:
            from apps.commerce.leads.bridges import create_lead_from_commerce_payment
        ...
        if phone:
            try:
                from apps.messaging.whatsapp.services import send_commerce_payment_receipt
        ...
        # (apps.commerce.products.signals.on_commerce_payment_completed).
