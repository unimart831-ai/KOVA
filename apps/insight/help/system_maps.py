...
""",
    ...
    SystemMap(
        ...
        audit=[
            ...
            "Builds content.MarketingCampaign + seeds → generate_from_seed.",
            "Can optionally create EmailCampaign alongside social posts.",
            "Legacy apps.core.campaigns kept in INSTALLED_APPS for migration history only.",
        ],
        diagram="""
flowchart LR
  VC[Voice Campaign UI] --> VB[VoiceBrief / prompt]
  VB --> AI[build MarketingCampaign]
  AI --> C[MarketingCampaign]
  C --> SEED[ContentSeeds]
...
""",
    ...
    SystemMap(
        ...
        title="Links & Business Hub",
        nav_group="Business",
        print_order=16,
        summary="Public link-in-bio and Business Hub under apps.commerce.links.",
        audit=[
            "Link-in-bio: /k/slug/ — KovaPage, forms → leads (apps.commerce.links).",
            "Business Hub: /p/slug/ — UserProfile page + AI salesperson (apps.commerce.links.hub).",
            "Both public URLs preserved; hub folded out of apps.kova_page.",
            "Form submissions can create Lead records.",
        ],
        diagram="""
flowchart LR
  subgraph links["Link-in-bio /k/"]
    LP[Link page builder] --> PUBK[Public /k/slug/]
    PUBK --> FORM[Form submit → Lead]
  end
  subgraph hub["Business Hub /p/"]
    PROF[UserProfile page settings] --> PUBP[Public /p/slug/]
    PUBP --> ASK[AI Salesperson]
