...
def _vision_context_from_asset(asset, user=None) -> dict:
    from apps.products.asset_metadata import normalize_asset_metadata
...
def propose_seeds_from_asset(asset, user, *, max_proposals: int = 5) -> list[SeedProposal]:
    ...
    from apps.content.campaign_attribution import rank_proposals_by_revenue_history
    from apps.media.revenue_dna import bias_proposals_with_revenue_dna, get_revenue_dna_hints
...
def _llm_enrich_proposals(asset, user, proposals: list[SeedProposal]) -> list[SeedProposal] | None:
    ...
    from apps.agents.llm import generate, parse_llm_json
