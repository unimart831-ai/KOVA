...
def attach_blueprint_to_seed(
    ...
    *,
    ...
    if resolved and resolved.asset_type == resolved.AssetType.SERVICE:
        from apps.content.service_templates import apply_service_template_to_blueprint, pick_service_template
        ...
        from apps.content.professional_templates import (
...
def blueprint_prompt_section(blueprint: dict) -> str:
    ...
    from apps.content.service_templates import service_template_prompt_lines

    template_lines = service_template_prompt_lines(blueprint)
    if template_lines:
        lines.append("")
        lines.append(template_lines)
    from apps.content.professional_templates import professional_template_prompt_lines
