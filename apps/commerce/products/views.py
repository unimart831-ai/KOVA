...
from apps.commerce.products.forms import BulkImportForm, ProductCategoryForm, ProductForm
from apps.commerce.products.models import Product, ProductCategory, RestockScan, StockAlert, StockUpdate
from apps.commerce.products.plan_gates import product_plan_context
from apps.commerce.products.stock_actions import log_stock_change, record_sale
...
def product_list(request):
    from apps.core.accounts.segments import build_surface_experience
    ...
    from apps.commerce.products.catalog_showcase import promotable_catalog_queryset
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if getattr(request.user.profile, "business_model", "") == "professional":
                from apps.commerce.products.professional_assets import apply_professional_asset_from_post
...
def product_detail(request, product_id):
    ...
    from apps.commerce.products.commerce_autopilot import commerce_autopilot_active, is_placeholder_product_name
    from apps.commerce.products.commerce_links import commerce_link_url, ensure_commerce_slug
    from apps.commerce.products.commerce_seo import commerce_seo_checklist, shop_index_url
    ...
    from apps.commerce.products.gallery_preferences import build_gallery_scenes, polish_actions_for_product
    from apps.commerce.products.photoroom_review import summarize_review_state
    from apps.commerce.products.photoroom_virtual_models import virtual_model_enabled
    from apps.commerce.products.scene_packs import marketplace_channel_image_urls
...
def restock_pipeline_status(request, scan_id):
    ...
    from apps.commerce.products.restock_pipeline import build_restock_pipeline_status
...
def product_edit(request, product_id):
    ...
    asset_ctx = {}
    if getattr(request.user.profile, "business_model", "") == "professional":
        from apps.commerce.products.professional_assets import asset_context_for_product
...
def catalog_showcase(request):
    ...
    from apps.commerce.products.tasks import create_catalog_showcase
...
def catalog_showcase_status(request, seed_id):
    ...
    from apps.create.content.models import ContentSeed
    from apps.commerce.products.catalog_showcase import build_catalog_showcase_status
...
def quick_post_product(request, product_id):
    ...
    from apps.commerce.products.tasks import quick_post_product_photo
    from apps.core.utils import fire_task
...
def reidentify_product(request, product_id):
    ...
    from apps.commerce.products.tasks import reidentify_product_from_photo
    from apps.core.utils import fire_task
...
def fix_and_promote(request, product_id):
    ...
    from apps.commerce.products.tasks import fix_and_promote_product
    from apps.core.utils import fire_task
...
def expand_product_photos_view(request, product_id):
    ...
    from apps.core.billing.visual_credits import check_visual_credit_limit
    from apps.commerce.products.photo_variations import VISUAL_MODE_PRO_SCENE, normalize_visual_mode
    from apps.commerce.products.tasks import expand_product_photo_set
    from apps.core.utils import fire_task
...
def set_gallery_hero_view(request, product_id):
    ...
    from apps.commerce.products.gallery_preferences import set_hero_image
...
def toggle_gallery_scene_view(request, product_id):
    ...
    from apps.commerce.products.gallery_preferences import set_scene_included
...
def review_variant_view(request, product_id):
    ...
    from apps.commerce.products.gallery_preferences import set_variant_review_status
    from apps.commerce.products.photoroom_review import review_alterations_enabled
...
def asset_intake(request):
    ...
    from apps.commerce.products.asset_intake import create_manual_asset
    from apps.commerce.products.models import BusinessAsset
...
def snap_to_sell(request):
    ...
    from apps.commerce.products.commerce_autopilot import commerce_autopilot_active
    from apps.commerce.products.photoroom import photoroom_enabled, studio_polish_unavailable_message
    from apps.commerce.products.polish_mode import LITE_POLISH_NOTICE, POLISH_MODE_LITE, studio_polish_unavailable
    from apps.commerce.products.scene_packs import SCENE_PACK_OPTIONS
    ...
    from apps.commerce.products.scene_packs import default_scene_pack_for_profile
...
def snap_launch(request):
    ...
    """
    from apps.core.billing.models import get_user_plan_limits
    from apps.commerce.products.commerce_autopilot import commerce_autopilot_active, sanitize_product_name
    from apps.commerce.products.image_utils import normalize_uploaded_image, validate_uploaded_images
    from apps.commerce.products.photo_variations import normalize_visual_mode
    ...
    if visual_mode == Product.VisualMode.PRO_SCENE:
        if polish_mode == POLISH_MODE_STUDIO:
            from apps.core.billing.visual_credits import check_visual_credit_limit
            from apps.commerce.products.photoroom import studio_polish_unavailable_message
    ...
    from apps.commerce.products.scene_packs import normalize_scene_pack, store_product_scene_pack
...
def snap_batch_transcribe(request):
    ...
    from apps.create.content.voice import transcribe_audio
...
def snap_batch_launch(request):
    ...
    from apps.commerce.products.models import BatchSnapSession
    from apps.commerce.products.tasks import snap_batch_process
    from apps.core.utils import fire_task
    ...
    product_ids = []
    form_prices = {}
    from apps.commerce.products.image_utils import normalize_uploaded_image
...
def restock_scan(request):
    ...
    from apps.commerce.products.models import RestockScan
    from apps.commerce.products.tasks import process_restock_scan
    from apps.core.utils import fire_task
...
def generate_product_video(request, product_id):
    ...
    from apps.commerce.products.tasks import generate_product_video_task
    from apps.core.utils import fire_task
...
def generate_promo_image(request, product_id):
    ...
    from apps.commerce.products.tasks import generate_promo_image_task
    from apps.core.utils import fire_task
...
def generate_virtual_model(request, product_id):
    ...
    from apps.core.billing.visual_credits import check_visual_credit_limit
    from apps.commerce.products.photoroom_virtual_models import virtual_model_enabled
    from apps.commerce.products.tasks import generate_virtual_model_task
    from apps.core.utils import fire_task
...
def generate_seasonal_variant(request, product_id):
    ...
    try:
        from apps.commerce.products.seasonal_variants import generate_seasonal_product_pack as gen_seasonal
...
def showcase_assets(request):
    ...
    from apps.create.briefs.asset_attribution import PROFESSIONAL_ASSET_TYPES
    from apps.commerce.products.models import BusinessAsset
    from apps.commerce.products.professional_assets import (
        create_case_study,
    ...
    if request.method == "POST":
        ...
        if action == "create_post":
            ...
            from apps.commerce.products.professional_assets import create_authority_post_from_asset
