...
def _apply_scene_prompt(resolved: dict[str, str], scene_prompt: str | None) -> dict[str, str]:
    if not scene_prompt:
        return resolved
    from apps.create.media.photoroom_brief import merge_brief_into_params
...
def resolve_variant_params(
    ...
    *,
    ...
    from apps.commerce.products.photoroom_brand_template import apply_brand_template
    ...
    for key, value in spec.params.items():
        if value == "{brand_color}":
            ...
            from apps.commerce.products.photoroom_api import relight_mode_for
    ...
    from apps.commerce.products.scene_packs import resolve_scene_vertical, vertical_locked_seed
...
def _scene_intelligence_context(
    ...
    *,
    ...
    from apps.commerce.products.scene_packs import stall_brief_scene_signals
...
def enforce_pack_capability_balance(
    ...
    *,
    ...
    """
    from apps.commerce.products.photoroom_api import HIGH_UNCERTAINTY_VARIANT_IDS
    from apps.commerce.products.photoroom_guard import uncertainty_is_high_for_category
...
def _boost_candidates_for_vertical(
    ...
    *,
    ...
    from apps.commerce.products.scene_packs import vertical_priority_variant_ids
...
def select_plus_variants(
    ...
    *,
    ...
    from apps.commerce.products.batch_snap_intelligence import is_market_day_mode
    ...
    from apps.commerce.products.scene_packs import resolve_scene_vertical, vertical_hero_studio_ids
    ...
    from apps.commerce.products.scene_packs import normalize_scene_pack, scene_pack_hero_studio_ids
    ...
    if vertical == "jewelry" or category == "jewelry":
        from apps.commerce.products.photoroom_guard import should_block_beautify
    ...
    if (
        ...
        from apps.commerce.products.scene_packs import vertical_commerce_scenes
    ...
    if visual_brief is not None:
        from apps.create.media.photoroom_brief import prioritize_brief_variants, variant_ids_from_brief
...
def multi_angle_polish_credit_enabled(plan_tier: str) -> bool:
    ...
    from apps.commerce.products.scene_packs import multi_angle_polish_enabled

    return multi_angle_polish_enabled(plan_tier)


def _api_key_headers(extra: dict | None = None) -> tuple[str | None, dict]:
    from apps.commerce.products.photoroom import _api_key_headers as base_headers
...
def _resolve_public_image_url(image_url: str) -> str | None:
    from apps.commerce.products.photoroom import _resolve_public_image_url as resolve

    return resolve(image_url)


def _load_image_bytes(image_url: str) -> tuple[bytes, str] | None:
    from apps.commerce.products.photoroom import _load_image_bytes as load
...
def run_plus_variant(
    ...
    *,
    ...
    from apps.create.media.photoroom_brief import scene_prompt_for_variant
    from apps.commerce.products.photoroom_api import PhotoroomEditResult
    from apps.commerce.products.photoroom_basic import is_basic_routable_variant, run_basic_white_cutout
