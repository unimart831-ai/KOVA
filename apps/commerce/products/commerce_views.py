...
from apps.commerce.products.commerce_canonical import build_unified_shop_footer
...
from apps.commerce.products.product_cta import (
...
def _track_commerce_view(request, product, profile):
    from apps.core.utils import run_task_inline
...
def _track_commerce_view_sync(product_pk, profile_pk, referrer: str):
    try:
        ...
        from apps.create.content.campaign_attribution import create_attributed_conversion, resolve_marketing_campaign
...
def _commerce_context(
    ...
    *,
    ...
    from apps.commerce.products.commerce_checkout import tracked_whatsapp_order_url, whatsapp_checkout_url
...
def public_shop_index(request, page_slug):
    ...
    from apps.commerce.products.commerce_cache import get_commerce_branding_cached, load_shop_auxiliary
    ...
    from apps.commerce.products.shop_flagship import (
...
def public_commerce_link(request, page_slug, commerce_slug):
    ...
    user = product.user
    brand = brand_name(profile, user)
    from apps.commerce.products.commerce_checkout import build_product_order_wa_text, checkout_heading_for
...
def public_whatsapp_order(request):
    ...
    from apps.commerce.products.commerce_wa_orders import resolve_whatsapp_order_redirect
