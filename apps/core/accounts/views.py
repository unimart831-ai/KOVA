...
from apps.core.accounts.facebook_oauth import start_facebook_platform_oauth

from apps.core.accounts.forms import (
...
from apps.core.accounts.onboarding_flow import (
    SETUP_TOTAL_STEPS,
...
def _is_facebook_phone_only_signup(user) -> bool:
    from apps.core.accounts.facebook_oauth import is_facebook_synthetic_email
...
def onboarding_choose_path(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            from apps.core.accounts.business_brain import extract_and_apply_brain
...
def ai_brand_builder(request):
    ...
    import json
    import logging
    from apps.create.agents.llm import generate, parse_llm_json, _get_llm_config
...
def onboarding_complete(request):
    ...
    from apps.core.accounts.setup_mission import (
        build_setup_mission,
...
def onboarding_progress_api(request):
    ...
    from apps.core.accounts.setup_mission import build_setup_mission, is_commerce_industry, get_onboarding_intent
...
def business_brain_view(request):
    ...
    """
    from apps.core.accounts.business_brain import brain_completeness, build_brain_snapshot
    from apps.create.agents.models import AgentAction
