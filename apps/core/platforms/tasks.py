...
def warn_expiring_tokens():
    ...
    for account in at_risk:
        ...
        if account.token_expires_at <= now:
            ...
            try:
                from apps.create.briefs.publish_notifications import notify_token_expiry_whatsapp
...
def poll_tiktok_publish_status():
    """V1 no-op — TikTok connect/publish is out of scope."""
    return {"polled": 0, "skipped": "tiktok_v1_disabled"}
