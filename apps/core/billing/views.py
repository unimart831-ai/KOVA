...
from apps.core.billing.forms import AgencySalesInquiryForm
from apps.core.billing.campaign_addons import user_can_purchase_addons
...
from apps.core.billing.services import (
...
def contact_sales(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            inquiry.user = user
            inquiry.save()
            from apps.core.billing.sales_inquiry import notify_staff_new_inquiry
...
def mpesa_checkout(request):
    ...
    from apps.core.billing.mpesa_services import activate_trial, initiate_mpesa_checkout
    from apps.core.billing.models import DiscountCode, DiscountRedemption
...
def mpesa_addon_checkout(request):
    ...
    from apps.core.billing.mpesa_services import initiate_mpesa_addon_checkout
...
def mpesa_webhook(request):
    ...
    """
    from apps.core.billing.models import MpesaPayment
    from apps.core.billing.mpesa import parse_stk_callback
    from apps.core.billing.mpesa_services import process_mpesa_callback
    ...
    try:
        ...
        if CommercePayment.objects.filter(checkout_request_id=checkout_id).exists():
            ...
            from apps.insight.analytics.webhooks import mpesa_commerce_callback
