...
def format_phone_number(phone):
    ...
    # Local 7xxxxxxxx → treat as Kenyan mobile before shared normalizer
    digits_only = raw.lstrip("+").replace(" ", "").replace("-", "")
    if digits_only.startswith("7") and len(digits_only) == 9:
        raw = "0" + digits_only

    formatted = phone_to_whatsapp_digits(raw)
    if not formatted.startswith("254") or len(formatted) != 12 or not formatted.isdigit():
