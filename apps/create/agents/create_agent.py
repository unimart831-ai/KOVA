...
from apps.create.agents.llm import generate, get_model_for_task, LLMAuthError, LLMResponse, parse_llm_json
...
def _get_performance_intelligence(user) -> str:
    ...
    """
    from apps.create.agents.analyst_agent import get_content_dna_summary
...
def build_system_prompt(user) -> str:
    ...
    from apps.create.content.post_copy import HUMAN_COPY_RULES
    ...
    from apps.commerce.products.utils import get_product_context
    ...
    from apps.create.agents.memory import get_agent_learning_context, get_user_edit_patterns
    ...
    from apps.create.agents.playbooks import get_playbook_intelligence
...
def build_generation_prompt(seed: ContentSeed, platforms: list[dict]) -> str:
    ...
    from apps.core.accounts.segments import build_segment_prompt_context, get_offer_prompt_guidance
...
"""
    ...
    if seed.product_id:
        ...
        from apps.commerce.products.utils import get_product_context, get_catalog_sampling_hint
...
"""
    ...
    if campaign or getattr(seed, "product_id", None):
        from apps.create.content.campaign_bundle import campaign_bundle_prompt_section
    ...
    if campaign or (getattr(seed, "blueprint", None) or {}).get("media_factory"):
        from apps.create.media.media_factory import media_factory_prompt_section
...
def build_fallback_posts(seed: ContentSeed, platforms: list[dict]) -> tuple[str, list[dict]]:
    ...
    from apps.create.content.campaign_bundle import post_format_allowed, content_types_from_seed
...
def run_create_agent(seed: ContentSeed, force_pending: bool = False) -> list[Post]:
    ...
    try:
        ...
        from apps.create.content.campaign_bundle import CONTENT_TYPE_ALL, content_types_from_seed
        ...
        from apps.create.content.platform_rewrite import apply_platform_rewrites
        ...
        from apps.create.content.campaign_bundle import content_types_from_seed, post_format_allowed
        ...
        for pd in post_dicts:
            ...
            from apps.create.content.tasks import sanitize_content, strip_markdown, _PLAIN_TEXT_PLATFORMS
            ...
            if seed and seed.product and seed.product.image:
                try:
                    from apps.create.content.models import Post as PostModel
            ...
            from apps.create.content.campaign_cta import commerce_url_for_first_comment
            ...
            if platform == "facebook" and not (post.first_comment or "").strip():
                from apps.core.utils.first_comments import compose_first_comment
            ...
            if has_visual_request and (platform_requires_media or user_opted_in_images) and not product_image_attached:
                from apps.core.billing.enforcement import check_ai_image_limit

                allowed, limit_msg = check_ai_image_limit(seed.user)
                if allowed:
                    ...
                    try:
                        ...
                        if post.post_format in ("story", "reel"):
                            from apps.create.content.tasks import generate_post_images
                            fire_task(generate_post_images, str(post.id))
                        else:
                            from apps.create.content.tasks import async_generate_image
                            fire_task(
                    ...
                    logger.info("Skipping visual gen for %s: %s", seed.user, limit_msg)
        ...
        from apps.create.content.campaign_bundle import ensure_campaign_bundle
