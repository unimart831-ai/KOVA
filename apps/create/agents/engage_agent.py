        ...
        for attempt in range(2):
            try:
                ...
                if (
                    ...
                    from apps.core.platforms.token_recovery import (
...
def generate_replies(user, batch_size=10):
    ...
    try:
        ...
        from apps.create.agents.engage_routing import safety_check, SafetyContext
        ...
        for interaction, payload in zip(batch_interactions, payloads):
            ...
            try:
                from apps.commerce.leads.bridges import create_lead_from_engage_intent
...
def auto_respond(user):
    ...
    for interaction in candidates:
        ...
        if action == RoutingAction.AUTO_SEND:
            from apps.core.billing.engage_trial import (
            ...
            if (interaction.ai_intent or "") == "complaint":
                try:
                    from apps.create.briefs.owner_alerts import notify_owner_complaint
...
def run_engage_cycle(user):
    ...
    dm_fetched = 0
    try:
        from apps.messaging.engage.dm_inbox import fetch_dms_for_user
