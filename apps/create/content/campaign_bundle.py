...
def _apply_post_defaults(post, seed, user, platform: str) -> None:
    ...
    from apps.create.content.campaign_cta import apply_default_campaign_cta, commerce_url_for_first_comment
    from apps.create.content.professional_cta import apply_professional_cta_to_post
    from apps.create.agents.create_agent import _get_kova_page_url
...
def _queue_bundle_media(post, seed, image_prompt: str, visual_strategy_data: dict | None = None) -> bool:
    ...
    allowed, _limit_msg = check_ai_image_limit(seed.user)
    if not allowed:
