...
def _score_brand(post, *, blueprint: dict | None) -> tuple[int, list[str]]:
    ...
    if blueprint:
        from apps.create.content.renderers import blueprint_quality_score
    ...
    from apps.create.content.safety import HALLUCINATION_PATTERNS
...
def _score_compliance(post) -> tuple[int, list[str]]:
    from apps.create.content.safety import ENGAGEMENT_BAIT_PATTERNS, HALLUCINATION_PATTERNS
...
def score_post_qa(post, *, seed=None, blueprint: dict | None = None) -> PostQAScore:
    ...
    from apps.create.content.platform_fit import enrich_post_qa_with_platform_fit
...
def audit_campaign_qa(campaign, posts: list, user) -> CampaignQAReport:
    ...
    from apps.create.content.platform_fit import PLATFORM_FIT_MIN_SCORE, platform_fit_gate
