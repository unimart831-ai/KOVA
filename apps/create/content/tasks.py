...
def _try_reel_carousel_fallback(post) -> bool:
    ...
    try:
        from apps.create.briefs.owner_alerts import queue_owner_alert
...
def _collect_reel_image_sources(post) -> list[str]:
    ...
    if post.product:
        from apps.create.content.product_visuals import polished_reel_sources
...
def post_generate_analytics(post_ids: list[str]):
    ...
    """
    from apps.create.content.models import Post
    from apps.create.agents.analyst_agent import batch_extract_content_dna, batch_predict_engagement
...
def async_generate_image(post_id: str, image_prompt: str, visual_strategy_data: dict | None = None):
    ...
    try:
        if visual_strategy_data and visual_strategy_data.get("strategy"):
            from apps.create.agents.visual_strategy import apply_visual_strategy
            ...
            from apps.create.agents.visual_strategy import infer_visual_strategy, apply_visual_strategy
...
def generate_from_seed(seed_id: str):
    ...
    from apps.commerce.products.utils import maybe_attach_sampled_product_to_autonomous_seed

    maybe_attach_sampled_product_to_autonomous_seed(seed)
    seed.refresh_from_db()

    from apps.create.content.campaigns import ensure_campaign_for_seed, sync_campaign_status_from_seed, update_campaign_quality_from_posts
    ...
    if campaign:
        ...
        try:
            from apps.create.content.campaign_whatsapp import notify_campaign_ready
    ...
    from apps.create.agents.adapt_agent import auto_schedule_post
...
def regenerate_post_async(post_id: str):
    ...
    """
    from apps.create.content.models import Post
    from apps.create.agents.create_agent import regenerate_single_post
    from apps.messaging.notifications.models import Notification
    ...
    try:
        ...
        from apps.create.content.models import PostVersion
...
def generate_post_images(post_id: str):
    ...
    """
    from apps.create.content.models import Post
    from apps.create.content.image_gen import generate_image, generate_carousel_images, generate_reel_images
    ...
    from apps.create.content.product_visuals import try_apply_product_polished_media
...
def _build_reel_hook_texts(post, meta: dict, slide_count: int) -> list[str]:
    ...
    if post.product:
        ...
        if not brand_name:
            from apps.commerce.products.commerce_seo import brand_name as resolve_brand_name
...
def _apply_reel_director(post, image_sources: list[str], meta: dict):
    ...
    from apps.create.content.reel_director import _sources_have_baked_captions, build_reel_plan
...
def compose_reel_video(post_id: str):
    ...
    from apps.create.content.reel_music import ensure_audio_bed, infer_mood_from_post, pick_music_track
    from apps.create.content.video_compose import VideoComposeError, compose_carousel_to_reel, compose_motion_reel
    ...
    if getattr(post, "seed_id", None):
        from apps.create.media.media_factory import apply_reel_strategy_to_post
    ...
    reel_plan = None
    from apps.create.content.reel_studio import build_professional_plan, professional_mode_enabled
    ...
    from apps.create.content.reel_music import resolve_track_path
    ...
    if meta.get("reel_template") == "carousel_to_video" or post.carousel_slides:
        ...
        from apps.create.content.reel_studio import sanitize_hooks_for_roles
        from apps.create.media.reel_strategy import ReelStrategy
        ...
        if rs:
            ...
            if reel_plan:
                from apps.create.content.reel_director import ReelComposePlan
    ...
    from apps.create.media.text_overlay import TextOverlayPass, apply_overlay_report_to_metadata
    ...
    from apps.create.media.content_types import ReelBackend
    from apps.create.media.router import reel_backend_for_post

    if reel_backend_for_post(post) == ReelBackend.KLING and not professional_mode_enabled():
        try:
            from apps.create.media.reel_bridge import try_kling_reel_for_post
    ...
    if use_photoroom:
        try:
            from apps.commerce.products.photoroom_video import try_photoroom_reel_for_post
    ...
    try:
        from apps.create.content.reel_frame_studio import brand_context_for_post
        from apps.create.content.video_compose import compose_from_plan
        ...
        mp4_bytes = None
        if reel_plan:
            from apps.create.media.remotion_bridge import compose_via_remotion
...
def publish_post(self, post_id: str):
    ...
    from apps.create.content.safety import is_publishing_paused
    ...
    if not (post.cta_url or "").strip():
        ...
        if seed:
            from apps.create.content.campaign_cta import apply_default_campaign_cta
            from apps.create.content.professional_cta import apply_professional_cta_to_post
    ...
    from apps.create.content.campaign_qa import check_post_publish_gate
    ...
    if not qa_allowed:
        ...
        from apps.create.briefs.publish_notifications import notify_publish_failure
    ...
    if review_blocked:
        from apps.commerce.products.photoroom_review import block_post_for_alteration_review
    ...
    if platform_name:
        from apps.core.platforms.outage import is_outage
    ...
    try:
        ...
        if token and token.startswith("gAAAAA"):
            from apps.core.platforms.encryption import decrypt_token
            logger.warning(
        ...
        from apps.core.platforms.providers.base import PlatformAuthError
        from apps.core.platforms.error_codes import translate_error
        from apps.core.platforms.outage import record_failure
    ...
    if result.success:
        ...
        from apps.create.briefs.publish_notifications import notify_publish_success
...
def check_and_publish_due_posts():
    ...
    from apps.core.accounts.autopilot_helpers import platform_allowed_for_autopilot, should_dispatch_due_post
...
def fetch_post_metrics(post_id: str):
    ...
    from apps.create.agents.memory import validate_prediction
...
def generate_ab_test_variants(ab_test_id: str):
    ...
    """
    from apps.create.content.models import ABTest
    from apps.create.agents.create_agent import generate_ab_variants
    from apps.create.agents.analyst_agent import extract_content_dna, predict_engagement
...
def evaluate_ab_tests():
    ...
    from datetime import timedelta
    from apps.create.content.models import ABTest
    from apps.create.agents.analyst_agent import evaluate_ab_test
...
def archive_expired_campaigns_task():
    ...
    from apps.create.content.campaign_archive import archive_expired_campaigns
...
def process_voice_brief(voice_brief_id: str):
    ...
    try:
        ...
        from apps.create.agents.llm_router import call_llm, call_whisper
