...
def regenerate_post(request, post_id):
    ...
    from apps.create.content.tasks import regenerate_post_async
...
def edit_post(request, post_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if content_changed:
                from apps.create.agents.memory import record_edit_feedback
...
def retry_image(request, post_id):
    ...
    from apps.create.content.tasks import retry_image_generation
...
def retry_reel(request, post_id):
    ...
    from apps.create.content.tasks import _queue_reel_compose
...
def reschedule_rate_limited(request, post_id):
    ...
    if request.headers.get("HX-Request"):
        ...
        from apps.create.content.views.card_helpers import render_post_card
...
def generate_image(request, post_id):
    ...
    from apps.core.billing.plan_limit_ui import plan_limit_banner_html, plan_limit_redirect

    allowed, limit_msg = check_ai_image_limit(post.user)
    if not allowed:
        if request.headers.get("HX-Request"):
...
def post_detail(request, post_id):
    ...
    if post.status in (
        ...
        try:
            from apps.create.content.quality_review import build_scorecard
