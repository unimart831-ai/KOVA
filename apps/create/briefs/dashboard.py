...
def _collect_home_stats(user, today, week_ago):
    ...
    try:
        ...
        pass
    try:
        from apps.commerce.leads.models import Lead, LeadEnrollment
...
def _setup_checklist_from_stats(user, stats):
    from apps.core.accounts.setup_mission import build_setup_mission
...
def get_money_board_stats(user):
    ...
    try:
        from apps.create.briefs.revenue_summary import get_unified_revenue_summary
...
def get_cached_home_extras(user, brief):
    ...
    from apps.create.briefs.views import (
        _build_brief_streak,
