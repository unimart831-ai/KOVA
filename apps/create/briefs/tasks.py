...
from apps.create.agents.analyst_agent import analyze_performance, get_content_dna_summary
...
from apps.create.agents.strategist_agent import get_engagement_report, run_strategy_cycle
from apps.insight.analytics.competitor_intel import get_competitor_context_for_brief
from apps.core.billing.models import get_plan_limits
from apps.create.briefs.models import DailyBrief
from apps.create.briefs.owner_alerts import send_owner_alert_task  # noqa: F401 — registers task on worker boot
...
def _gather_overnight_work(user):
    ...
    from apps.create.briefs.operations_report import enrich_overnight_work
    from apps.create.content.models import Post, ContentSeed
...
def _gather_brief_data(user):
    ...
    try:
        from apps.create.agents.models import AgentAction as _AA
    ...
    try:
        from apps.commerce.products.utils import get_product_brief_data
    ...
    try:
        from apps.insight.analytics.revenue import get_revenue_brief_data, get_revenue_headline_insight
    ...
    try:
        from apps.create.briefs.calendar_hints import upcoming_moments
...
def _generate_brief_with_llm(user, brief_data):
    ...
    system_prompt = (
        ...
        '2-3 short sentences.\n'
        '  Paragraph 3 (LOOKING AHEAD): A forward-looking strategic thought — '
        'what pattern is emerging, what opportunity you see, or what the owner '
        'should keep in mind for the week. NOT a to-do item (those go in the '
        'Decision Stream which the owner sees separately). 1-2 sentences.\n'
        '- "decisions_needed": list of 0-3 STRATEGIC items needing human judgment, '
        'each with {item, context, recommended_action, urgency: "now"|"today"|"this_week"}. '
        'IMPORTANT: Do NOT include items already covered by live data — pending posts, '
        'flagged interactions, new leads, failed posts, and pending bookings are shown '
        'from the database in a separate Decision Stream. Only include judgment calls '
        'that require strategic thinking: e.g. "Consider switching posting frequency", '
        '"Competitor X launched a campaign — worth responding?", '
        '"Brand voice feels inconsistent across platforms". '
        'Empty list if nothing needs strategic attention.\n'
        ...
        '- "escalations": [] (DEPRECATED — always return empty list. '
        'Escalations are now handled by the deterministic Decision Stream.)\n'
...
def generate_daily_brief(user, *, user_date=None):
    ...
    try:
        ...
        from apps.create.briefs.delivery import deliver_daily_brief
...
def send_money_board_digests():
    ...
    from apps.create.briefs.revenue_summary import format_money_board_digest
