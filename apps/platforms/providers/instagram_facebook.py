...
# Basic scopes — always available without App Review
FB_SCOPES_BASIC = "email,public_profile"

# Page management scopes — enable these in Meta Developer Portal → Permissions & Features
# before adding them here. Requires App Review for production.
FB_SCOPES_PAGES = "pages_show_list,pages_manage_posts,pages_read_engagement"

# Instagram scopes — enable in Meta portal under Instagram use case
FB_SCOPES_INSTAGRAM = "instagram_basic,instagram_content_publish"

# Build final scope string from what's configured in settings
# Set FACEBOOK_PAGE_SCOPES_ENABLED = True in settings once you enable them in Meta portal
FB_SCOPES = FB_SCOPES_BASIC
if getattr(settings, "FACEBOOK_PAGE_SCOPES_ENABLED", False):
    FB_SCOPES += "," + FB_SCOPES_PAGES
if getattr(settings, "FACEBOOK_INSTAGRAM_SCOPES_ENABLED", False):
    FB_SCOPES += "," + FB_SCOPES_INSTAGRAM
