...
class TestNurtureWhatsAppUI:
    ...
    def test_growth_user_can_create_whatsapp_step(self, client, growth_user):
        ...
        resp = client.post(url, {
            ...
            "step_0_delay": 1,
            "step_0_action": "send_whatsapp",
            "step_0_body": "Karibu from Kazi plan!",
        })
        assert resp.status_code == 302
        seq = NurtureSequence.objects.get(user=growth_user, name="Growth WA")
        assert seq.steps.get().action_type == NurtureStep.ActionType.SEND_WHATSAPP
    ...
    def test_form_shows_whatsapp_option_for_growth(self, client, growth_user):
        client.force_login(growth_user)
        resp = client.get(reverse("leads:nurture_create"))
        assert resp.status_code == 200
        assert b"send_whatsapp" in resp.content
